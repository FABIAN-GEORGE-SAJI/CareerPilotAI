...
# 2. Guard & UI Setup
require_login("pages/5_Resume_Rewrite.py")
apply_theme()
render_sidebar()
...
"""
    ...
    <div style="background:rgba(255,255,255,.02); border:1px solid rgba(255,255,255,.08); padding:20px; border-radius:16px; margin-bottom:16px;">
        <p style="color:#A8B8D0; margin-bottom:12px;">Download your optimized resume as plain text, or as a formatted Word document ready to send.</p>
    </div>
    """,
        unsafe_allow_html=True,
    )
    ...
    with export_col2:
        try:
            ...
            doc.add_paragraph(result.get("professional_summary", ""))
            ...
            experience = result.get("improved_experience", [])
            if experience:
                ...
                for exp in experience:
                    ...
                    run.bold = True
                    run.font.size = Pt(12)
                    doc.add_paragraph(exp.get("rewritten_description", ""))

            projects = result.get("improved_projects", [])
            if projects:
                ...
                for project in projects:
                    ...
                    run.bold = True
                    run.font.size = Pt(12)
                    doc.add_paragraph(project.get("rewritten_description", ""))

            keywords = result.get("added_keywords", [])
            if keywords:
                doc.add_heading("ATS Keywords", level=1)
            ...
            st.caption("Install `python-docx` to enable formatted Word export.")
